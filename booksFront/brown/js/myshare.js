...
var app = new Vue({
  ...
  created() {
    ...
    $.ajax({
      url: "http://localhost:9090/showShareUserData",
      method: "POST",
      data: {
        username: username
      },
    ...
    $.ajax({
      url: "http://localhost:9090/showShareUser",
      method: "POST",
      data: {
        username: username
      },
  ...
  methods: {
    ...
    submitWord(event, kind) {
      if (kind == 0) {
        if (this.file == "") {
          ...
          // const URLIMG = "http://localhost:9090/upload";
          $.ajax({
              url: URLIMG,
              type: "POST",
              cache: false,
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let urlHttp = "http://localhost:9090/addShareUserData";
    ...
    downshare(shareid) {
      let _bm = this;
      $.ajax({
        url: "http://localhost:9090/shareUserdownoreup",
        method: "POST",
        data: {
          id: shareid,
          downup: 101
